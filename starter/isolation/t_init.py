...
def _play(agents, game_state, time_limit, match_id, debug=False):
    """
    Run a match between two agents by alternately soliciting them to select a move and applying it to advance the game state.
    :param agents: tuple
        agents[i] is an instance of isolation.Agent class (namedtuple)
    :param game_state: Isolation
         an instance of Isolation.Isolation in the initial game state;
         assumes that agents[game_state.ply_count % 2] is the active player in the initial state
    :param time_limit: numeric
        The maximum number of milliseconds to allow before timeout during each turn (see notes)

    :return:
    (agent, list<[(int, int),]>, Status)
        return multiple including the winning agent, the actions that were applied to the initial state, a status code describing the reason the game ended, and any error information
    """
    ...
    while not game_state.terminal_test():
        ...
        try:
            ...
            The queue was empty after get_action() was called. This means that either the queue.put() method was not called by the get_action() method, or that the queue was empty after the procedure was killed due to timeout {} seconds after the move time limit of {} milliseconds had expired
...
def _request_action(agent, queue, game_state):
    """
    Augment agent instances with a countdown timer on every method before calling the get_action() method and catch countdown timer exceptions
    """
    agent.queue = queue
    queue.agent = agent
    try:
