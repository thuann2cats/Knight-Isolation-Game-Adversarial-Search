...
def _play(agents, game_state, time_limit, match_id, debug=False):
    ...
    while not game_state.terminal_test():
        ...
        # us is GREEN, opponent is RED
        state_str_representation = DebugState.from_state(game_state).__str__()
        char_index_p1 = state_str_representation.find("1")
        char_index_p2 = state_str_representation.find("2")
        smaller_index = char_index_p1
        larger_index = char_index_p2
        # color_smaller_index = Fore.RED
        # color_larger_index = Fore.GREEN
        if char_index_p1 > char_index_p2:
            smaller_index = char_index_p2
            larger_index = char_index_p1
            # color_smaller_index = Fore.GREEN
            # color_larger_index = Fore.RED
        if agents[0].name == "Custom Agent":
            # meaning p1 is our agent
            if char_index_p1 < char_index_p2:
                color_smaller_index = Fore.GREEN
                color_larger_index = Fore.RED
            else:
        ...
        print(state_str_representation[:smaller_index], end="")
        print(color_smaller_index + "\033[1m" + state_str_representation[smaller_index] + Style.RESET_ALL, end="")
        print(state_str_representation[smaller_index+1: larger_index], end="")
        print(color_larger_index + "\033[1m" + state_str_representation[larger_index] + Style.RESET_ALL, end="")
        print(state_str_representation[larger_index+1:])
